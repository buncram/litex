...
class VexiiRiscv(CPU, AutoDoc):
    ...
    io_regions           = {
        # Origin, Length.
        0xb000_0000 : 0x0001_0000,
        0xE000_0000 : 0x2000_0000,
        0x9000_0000 : 0x0010_0000,
    }
...
""")
        ...
        if False:
            ...
            # Create a crossbar to merge icache and dcache access
            mem64_xbar = AXICrossbar(platform=platform)
            self.submodules += mem64_xbar
            mem64_xbar.add_slave(name = "cpu_d", s_axi=self.dbus_axi,
            ...
            mem64_xbar.add_slave(name = "cpu_i", s_axi=self.ibus_axi,
                # aw_reg = AXIRegister.BYPASS,
                # w_reg  = AXIRegister.BYPASS,
                # b_reg  = AXIRegister.BYPASS,
                # ar_reg = AXIRegister.BYPASS,
            ...
            mem64_iface = axi.AXIInterface(data_width=64, address_width=32, id_width=4, bursting=True)
            mem64_xbar.add_master(name = "peripherals", m_axi=mem64_iface, origin=0,
                size=0x1_0000_0000)
            # adapt down to 32 bits
            mem32_iface = axi.AXIInterface(data_width=32, address_width=32, id_width=4, bursting=True)
            self.submodules += AXIAdapter(platform, s_axi = mem64_iface, m_axi = mem32_iface)

            # mux with uncached accesses
            mem32_xbar = AXICrossbar(platform=platform)
            self.submodules += mem32_xbar
            mem32_xbar.add_slave(name = "mem", s_axi=mem32_iface)
            mem32_xbar.add_slave(name = "peri", s_axi=self.pbus_axi)
            merged32 = axi.AXIInterface(data_width=32, address_width=32, id_width=5, bursting=True)
            mem32_xbar.add_master(name = "bus", m_axi=merged32, origin=0, size=0x1_0000_0000)

            # convert to axil
            peri_axil = axi.AXILiteInterface(name="bus_axil", bursting = False)
            self.submodules += AXI2AXILiteAdapter(platform, merged32, peri_axil)

            # Expose AXI-Lite Interfaces.
            self.periph_buses     = [peri_axil] # Peripheral buses (Connected to main SoC's bus). Leave blank because we don't want any bus to me inferred for the core generation.
            self.memory_buses     = []
        else:
            IBUS_ID = Signal(2, reset=0)
            DBUS_ID = Signal(reset=1)
            ...
            mem64_iface = axi.AXIInterface(data_width=64, address_width=32, id_width=4, bursting=True)
            mem64_xbar.add_master(name = "merged_mem", m_axi=mem64_iface, origin=0,
                size=0x1_0000_0000)

            # Convert mery to axil
            cpu_axil = axi.AXILiteInterface(name="cpu_axil", data_width=64, bursting = False)
            self.submodules += AXI2AXILiteAdapter(platform, mem64_iface, cpu_axil)

            # adapt memory down to 32 bits
            mem32_iface = axi.AXILiteInterface(data_width=32, address_width=32, bursting=False)
            self.submodules += AXILiteAdapter(platform, s_axil = cpu_axil, m_axil = mem32_iface)
            ...
            # Expose AXI-Lite Interfaces.
            self.periph_buses     = [peri_axil, mem32_iface] # Peripheral buses (Connected to main SoC's bus). Leave blank because we don't want any bus to me inferred for the core generation.
            self.memory_buses     = []

        self.reset = Signal()
        self.reset_address = 0x8000_0000 # hard coded from vexiiriscv spinal code
        ...
        platform.add_source_dir("VexiiRiscv/VexiiRiscv.sv")
    ...
    def add_sources(platform, variant="standard"):
        platform.add_source("VexiiRiscv/VexiiRiscv.sv")
